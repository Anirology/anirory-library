...
export default function BookDetails({ book, loading, onClose, onEdit, onDelete }) {
  return (
    <ModalShell title="Book details" onClose={onClose} className="details-modal">
      {loading ? <div className="details-skeleton skeleton" /> : book && <>
        ...
        <div className="modal-actions"><button className="button danger ripple" onClick={() => onDelete(book)}><Trash2 aria-hidden="true" />Delete</button><button className="button primary ripple" onClick={() => onEdit(book)}><Edit3 aria-hidden="true" />Edit record</button></div>
      </>}
    </ModalShell>
  )
}
